            ...
            {                
                ...
                for (First_round=0; ; First_round++)
                {
                    ...
                    for (int i = 0; i < Count_Points - 1; i++)                 
                        x0[i+1] = x0[i] + Delta_border_T * u0[i];                    
                    ...
                    for(int Second_round=0; Second_round<1;)
                    {
                        ...
                        lambda[Count_Points - 1] = 0 + Null_lambda * Delta_border_T * Delta_border_T * 0.999;
                        for (int i = Count_Points - 2; i > 0; i--)
                    ...
                    else
                    {
                        ...
                        {
                            J_delta_T[First_round % 2] = J_delta_T[First_round % 2];
                            for (int j = 0; j < Count_Points; j++)
                            {
                                x_delta_T[First_round % 2, j] = x_delta_T[First_round % 2, j];
                                u_delta_T[First_round % 2, j] = u_delta_T[First_round % 2, j];
                            }
                        }
                    }
                    if (Count_Points > 1000000)
                         break;
                ...
                textBox1.Text = $"{First_round}";
                textBox2.Text = $"{J_delta_T[First_round % 2]}";
                int s = 0;
                for (double k = Left_border; k < Right_border; k += 0.001)
                {
                    chart1.Series[0].Points.AddXY(k, x_delta_T[First_round % 2, s]);
                    chart1.Series[1].Points.AddXY(k, u_delta_T[First_round % 2, s]);
                    //chart1.Series[2].Points.AddXY(k, ((Math.Pow(x_delta_T[First_round % 2, s], 2) + Math.Pow(u_delta_T[First_round % 2, s], 2)) / 2) + 2 * u_delta_T[First_round % 2, s] * lambda[s]);
                    chart1.Series[2].Points.AddXY(k, Math.Pow(1 - u_delta_T[First_round % 2, s], 2) / -16 + Delta_border_T * k * x_delta_T[First_round % 2, s] + u_delta_T[First_round % 2, s] * lambda[s]);
